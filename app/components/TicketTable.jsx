 ...
 *
 * NOTE: This component renders the `tickets` prop directly.
 * Optimistic updates (status/team changes) are tracked via a local
 * `overrides` map so they don't conflict with parent-controlled filtering.
 */
...
export default function TicketTable({
  tickets,                  // controlled by parent — always reflects current filter
  isAdmin = false,
  onClearFilters,
  hasActiveFilters = false,
}) {
  // Optimistic local overrides: { [ticketId]: { status?, assignedTeam? } }
  const [overrides, setOverrides] = useState({});
  const [updating, setUpdating] = useState(null);
  const [expandedId, setExpandedId] = useState(null);
  const [error, setError] = useState('');

  // Merge a ticket with any local override
  ...
  const handleStatusChange = async (ticketId, newStatus) => {
    ...
    try {
      ...
      // Store optimistic override so the row reflects the change immediately
      setOverrides((prev) => ({
        ...prev,
  ...
  const handleTeamChange = async (ticketId, newTeam) => {
    ...
    try {
      ...
      setOverrides((prev) => ({
        ...prev,
        [ticketId]: { ...prev[ticketId], assignedTeam: newTeam },
      }));
    } catch (err) {
      setError(err.message);
    } finally {
        ...
        {hasActiveFilters ? (
          <>
            ...
            <p className="text-sm mt-1 mb-4 text-gray-500">
              Try adjusting or clearing your filters to see tickets.
            </p>
            {onClearFilters && (
                      ...
                      <td className="py-3 pr-4" onClick={(e) => e.stopPropagation()}>
                        {updating === ticket.id ? (
                          ...
                          <select
                            ...
                            className="text-xs border border-gray-200 rounded-md px-2 py-1 bg-white focus:outline-none focus:ring-1 focus:ring-blue-400 cursor-pointer"
                            aria-label={`Reassign team for ${ticket.subject}`}
                          >
                            {TEAM_OPTIONS.map((t) => (
