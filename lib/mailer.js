...
async function send({ to, subject, html }) {
  ...
  if (!transport) {
    console.warn(`[mailer] SMTP not configured (SMTP_USER/SMTP_PASS missing) — skipping email to ${to} | subject: "${subject}"`);
    return;
  }
  try {
    ...
    console.error(`[mailer] ✗ Failed to send "${subject}" → ${to} | error: ${err.message}`, err);
  }
}
...
export async function sendTicketCreatedClient(ticket, clientEmail) {
  console.log(`[mailer] sendTicketCreatedClient called | ticketId: ${ticket?.id} | clientEmail: ${clientEmail ?? 'none'}`);
  if (!clientEmail) { console.warn('[mailer] sendTicketCreatedClient skipped — no clientEmail'); return; }
  await send({
    to:      clientEmail,
...
export async function sendTicketCreatedAdmin(ticket, clientEmail) {
  ...
  console.log(`[mailer] sendTicketCreatedAdmin called | ticketId: ${ticket?.id} | adminEmail: ${adminEmail ?? 'not set'}`);
  if (!adminEmail) { console.warn('[mailer] sendTicketCreatedAdmin skipped — ADMIN_EMAIL not set'); return; }
...
export async function sendTicketResolvedClient(ticket) {
  console.log(`[mailer] sendTicketResolvedClient called | ticketId: ${ticket?.id} | clientEmail: ${ticket?.clientEmail ?? 'none'}`);
  if (!ticket.clientEmail) { console.warn('[mailer] sendTicketResolvedClient skipped — ticket has no clientEmail'); return; }
  await send({
...
export async function sendTicketResolvedAdmin(ticket, actor) {
  const adminEmail = process.env.ADMIN_EMAIL;
  console.log(`[mailer] sendTicketResolvedAdmin called | ticketId: ${ticket?.id} | actor: ${actor?.email ?? 'none'} | adminEmail: ${adminEmail ?? 'not set'}`);
  if (!adminEmail) { console.warn('[mailer] sendTicketResolvedAdmin skipped — ADMIN_EMAIL not set'); return; }
  ...
  if (actor?.email && actor.email.toLowerCase() === adminEmail.toLowerCase()) {
    console.log('[mailer] sendTicketResolvedAdmin skipped — actor is the admin, resolver email will be sent instead');
    return;
  }
  await send({
    to:      adminEmail,
...
export async function sendTicketResolvedResolver(ticket, actor) {
  console.log(`[mailer] sendTicketResolvedResolver called | ticketId: ${ticket?.id} | actor: ${actor?.email ?? 'none'}`);
  if (!actor?.email) { console.warn('[mailer] sendTicketResolvedResolver skipped — actor has no email'); return; }
  await send({
    to:      actor.email,
...
export async function sendChangeNotificationActor(ticket, actor, changes) {
  console.log(`[mailer] sendChangeNotificationActor called | ticketId: ${ticket?.id} | actor: ${actor?.email ?? 'none'} | changes: ${changes?.map(c => c.label).join(', ')}`);
  if (!actor?.email) { console.warn('[mailer] sendChangeNotificationActor skipped — actor has no email'); return; }
...
export async function sendDetailsRequestedClient(ticket, clientEmail, message, actor) {
  console.log(`[mailer] sendDetailsRequestedClient called | ticketId: ${ticket?.id} | clientEmail: ${clientEmail ?? 'none'} | requestedBy: ${actor?.email ?? 'none'}`);
  if (!clientEmail) { console.warn('[mailer] sendDetailsRequestedClient skipped — no clientEmail'); return; }
  await send({
    to:      clientEmail,
