...
const dashboardBtn = () => {
  const url = `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/login`;
  return `
...
export async function sendTicketSignedOff(ticket, admin, resolver) {
  ...
  // Admin + Resolver internal notification — separate buttons for each
  ...
  const resolverAppUrl = process.env.NEXTAUTH_URL || 'http://localhost:3000';
  const resolverHtml = base(`
    <h2 style="color:#1e293b;font-size:18px;margin:0 0 8px;">Ticket signed off ✅</h2>
    <p style="color:#475569;margin:0 0 20px;">
      <strong>${admin.name}</strong> (Admin) has signed off ticket #${formatTicketNumber(ticket.ticketNumber)} that you resolved.
      The client has been notified.
    </p>
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px;margin-bottom:16px;">
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ...
        <tr><td style="color:#94a3b8;padding:4px 0;">Subject</td>
            <td style="color:#1e293b;font-weight:600;">${ticket.subject}</td></tr>
        <tr><td style="color:#94a3b8;padding:4px 0;">Resolved by</td>
            <td style="color:#1e293b;">You (${resolver?.name})</td></tr>
        <tr><td style="color:#94a3b8;padding:4px 0;">Signed off by</td>
            <td style="color:#1e293b;">${admin.name}</td></tr>
      </table>
    </div>
    <div style="text-align:center;margin-top:24px;padding-top:20px;border-top:1px solid #e2e8f0;">
      <a href="${resolverAppUrl}/dashboard"
        style="display:inline-block;background:#2563eb;color:#fff;text-decoration:none;font-weight:600;font-size:14px;padding:10px 28px;border-radius:8px;">
        View My Dashboard →
      </a>
    </div>
  ...
  if (adminEmail) results.push(send({ to: adminEmail, subject: `[HelpDesk] Ticket signed off — ${ticket.subject}`, html: adminHtml }));
  if (resolver?.email && resolver.email.toLowerCase() !== adminEmail?.toLowerCase()) {
    results.push(send({ to: resolver.email, subject: `[HelpDesk] Your resolved ticket was signed off — ${ticket.subject}`, html: resolverHtml }));
  }
...
// ─── Template 15: Intervention Request ───────────────────────────────────────

export async function sendInterventionRequest(ticket, requester, recipient, message) {
  if (!recipient?.email) return { sent: false, reason: 'no_recipient_email' };
  const appUrl = process.env.NEXTAUTH_URL || 'http://localhost:3000';
  const ticketUrl = `${appUrl}/tickets/${ticket.id}`;
  return send({
    to:      recipient.email,
    subject: `[HelpDesk] Intervention needed on ticket #${formatTicketNumber(ticket.ticketNumber)} — ${ticket.subject}`,
    html: base(`
      <h2 style="color:#1e293b;font-size:18px;margin:0 0 8px;">Intervention requested 🆘</h2>
      <p style="color:#475569;margin:0 0 20px;">
        <strong>${requester.name}</strong> (${requester.team || requester.role}) is requesting your assistance
        on a support ticket and needs your intervention to move forward.
      </p>

      <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:16px;margin-bottom:16px;">
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          <tr><td style="color:#94a3b8;padding:4px 0;width:110px;">Ticket #</td>
              <td style="font-family:monospace;font-weight:700;color:#c2410c;">#${formatTicketNumber(ticket.ticketNumber)}</td></tr>
          <tr><td style="color:#94a3b8;padding:4px 0;">Subject</td>
              <td style="color:#1e293b;font-weight:600;">${ticket.subject}</td></tr>
          <tr><td style="color:#94a3b8;padding:4px 0;">Requested by</td>
              <td style="color:#1e293b;">${requester.name} · ${requester.team || requester.role}</td></tr>
          ${ticket.clientEmail ? `<tr><td style="color:#94a3b8;padding:4px 0;">Client</td>
              <td style="color:#1e293b;">${ticket.clientEmail}</td></tr>` : ''}
        </table>
      </div>
      ...
      ${message ? `
      <div style="border-left:4px solid #f97316;padding:14px 16px;background:#fff7ed;border-radius:0 8px 8px 0;margin-bottom:20px;">
        <p style="margin:0 0 6px;font-size:12px;color:#ea580c;font-weight:700;text-transform:uppercase;">Message from ${requester.name}</p>
        <p style="margin:0;color:#9a3412;font-size:14px;line-height:1.6;">${message}</p>
      </div>` : ''}

      <div style="text-align:center;margin-top:24px;padding-top:20px;border-top:1px solid #e2e8f0;">
        <a href="${ticketUrl}"
          style="display:inline-block;background:#ea580c;color:#fff;text-decoration:none;font-weight:600;font-size:14px;padding:10px 28px;border-radius:8px;">
          View Ticket →
        </a>
      </div>
    `),
